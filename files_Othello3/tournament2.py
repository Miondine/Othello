...
class Tournament:
    ...
    def __init__(self, tournament_name, type_player1, name_player1, depth_player1 = None, opponent_types = 'all', num_cycles = 10, num_games = 100, max_depth = 7, opening_book = False):
    ...
    def init_player_file(self,name_player):
        ...
        player_filename = f'{name_player}_results_{self.tournament_name}.txt'
        try:
    ...
    def play(self):
        ...
        for type_player2 in self.opponents_types:

            if(self.type_player1 in Tournament.players_simple):            
                if(type_player2 in Tournament.players_simple):
                    self.play_stats(type_player2,Tournament.player_names[type_player2],depth_player2 = None)
                elif(type_player2 in Tournament.players_minimax):
                    for depth in range(7,self.max_depth + 1):
                        self.play_stats(type_player2,f'{Tournament.player_names[type_player2]}{depth}',depth_player2 = depth)
                else:
                    for depth in range(3,self.max_depth):
                        self.play_stats(type_player2,f'{Tournament.player_names[type_player2]}{depth}',depth_player2 = self.mcts_iterations[depth])
            else:
                if(type_player2 in Tournament.players_simple):
                    self.play_stats(type_player2,Tournament.player_names[type_player2],depth_player2 = None)
                elif(type_player2 in Tournament.players_minimax):
                    for depth in range(6,self.max_depth + 1):
                        if(self.opening_book == True):
                            self.play_stats_opening(type_player2,f'{Tournament.player_names[type_player2]}{depth}',depth_player2 = depth)
                        else:
                            self.play_short(type_player2,f'{Tournament.player_names[type_player2]}{depth}',depth_player2 = depth)
                else:
                    for depth in range(self.max_depth):
                        if(self.opening_book == True):
                            self.play_stats_opening(type_player2,f'{Tournament.player_names[type_player2]}{depth}',depth_player2 = self.mcts_iterations[depth])
                        else:
                            self.play_short(type_player2,f'{Tournament.player_names[type_player2]}{depth}',depth_player2 = self.mcts_iterations[depth])
